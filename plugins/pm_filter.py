...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>အ့တာအတွက် ရှာပေးနေသည် {message.text} 🔍</i></b>")
        ...
        if total_results == 0:
            ...
            return await message.reply_text(f"<b>Hᴇʏ {message.from_user.mention}, {str(total_results)} {search}.သင့်ရှာဖွေမှုအတွက် ရလဒ်များကို ကျွန်ုပ်၏ ဒေတာဘေ့စ်တွင်တွေ့ရှိပါသည်။\n\nဤသည်မှာ ပံ့ပိုးမှုအဖွဲ့တစ်ခုဖြစ်သည့်အတွက် ဤနေရာမှ ဖိုင်များကို ရယူနိုင်မည် မဟုတ်ပါ...\n\nJᴏɪɴ Aɴᴅ Sᴇᴀʀᴄʜ Hᴇʀᴇ - {GRP_LNK}</b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>အ့တာအတွက် ရှာပေးနေသည် {content} 🔍</i></b>", reply_to_message_id=message.id)
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>အ့တာအတွက် ရှာပေးနေသည် {movie} 🔍</i></b>")
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("⛑️ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/moviemmmsb_gp")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("⛑️ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/moviemmsb_gp")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
                    ...
                    if fileid == "None":
                        ...
                        if manual == False:
                            ...
                            try:
                                ...
                                if settings['auto_ffilter']:
                                    ai_search = True
                                    reply_msg = await message.reply_text(f"<b><i>{message.text} အ့တာအတွက် ရှာပေးနေသည် 🔍</i></b>")
                                    await auto_filter(client, message.text, message, reply_msg, ai_search)
                        else:
                            try:
                                if settings['auto_delete']:
