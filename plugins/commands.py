...
async def start(client, message):
   ...
   # Progress bar animation
    ...
    await baby.edit_text(f"**🩵🤗 ကြိုဆိုပါတယ် {message.from_user.mention}**")
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('˹ ᴧʟʟ ˼', url=CHNL_LNK)
        ]]
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgEAAxkBAAEPz5lpHKOshHgqqkJxuYgCzdVpz8ZdugACOQIAAm6B2USQaKQpynaQzDYE") 
        await asyncio.sleep(1)
        await m.delete()
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            await asyncio.sleep(1) 
        await sts.delete()
        k = await client.send_message(chat_id = message.from_user.id, text=f"<blockquote><b><u>❗️❗️❗️သတိပေးချက်❗️️❗️❗️</u></b>\n\n10 မိနစ်အတွက် ဤမက်ဆေ့ကိုဖျက်မည် <b><u> မဖျက်ချင် တနေရာရာတွင် Forwardလုပ်ပီးသိမ်းထားပါ</u> </b></blockquote>")
        await asyncio.sleep(600)
        for x in filesarr:
            await x.delete()
        await k.edit_text("<b>✅ သင့်မက်ဆေ့ကို အောင်မြင်စွာ ဖျက်ပြီးပါပြီ။ နောက်တစ်ကြိမ် လိုချင်ပါက အောက်ပါခလုတ်ကို နှိပ်ပါ။</b>")  
        return
    ...
    if data.startswith("sendfiles"):
        ...
        if settings['is_shortlink'] and not await db.has_premium_access(user):
            ...
            text = "<b>✅ Yᴏᴜʀ Fɪʟᴇ Rᴇᴀᴅʏ Cʟɪᴄᴋ Oɴ Dᴏᴡɴʟᴏᴀᴅ Nᴏᴡ Bᴜᴛᴛᴏɴ Tʜᴇɴ Oᴘᴇɴ Lɪɴᴋ Tᴏ Gᴇᴛ Fɪʟᴇ\n\n</b>"
            if PREMIUM_AND_REFERAL_MODE == True:
                text += "<b>အတည်ပြုချက်မလိုဘဲ တိုက်ရိုက်ဖိုင်များ လိုချင်ပါက Premium ဝယ်ယူပါ ☺️\n\n💶 Premium ဝယ်ယူရန် /plan ကိုပို့ပါ</b>"
            k = await client.send_message(chat_id=message.from_user.id, text=text, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(1200)
            await k.edit("<b>✅ Yᴏᴜʀ Mᴇssᴀɢᴇ Is Sᴜᴄᴄᴇssғᴜʟʟʏ Dᴇʟᴇᴛᴇᴅ</b>")
            return
    ...
    if not files_:
        ...
        try:
            ...
            btn = [[InlineKeyboardButton("♻️ ဖိုင်ထပ်ယူမည်", callback_data=f'del#{file_id}')]]
            k = await msg.reply(text=f"<blockquote><b><u>❗️❗️❗️သတိပေးချက်❗️️❗️❗️</u></b>\n\n10 မိနစ်အတွက် ဤမက်ဆေ့ကိုဖျက်မည် <b><u> မဖျက်ချင် တနေရာရာတွင် Forwardလုပ်ပီးသိမ်းထားပါ</u> </b></blockquote>")
            await asyncio.sleep(600)
            await msg.delete()
            await k.edit_text("<b>✅ သင့်မက်ဆေ့ကို အောင်မြင်စွာ ဖျက်ပြီးပါပြီ။ နောက်တစ်ကြိမ် လိုချင်ပါက အောက်ပါခလုတ်ကို နှိပ်ပါ။</b>",reply_markup=InlineKeyboardMarkup(btn))
            return
        except:
            pass
...
async def requests(bot, message):
    ...
    if success:
        ...
        await message.reply_text("<b>တောင်းဆိုချက်ကို ထည့်သွင်းပြီးပါပြီ! ခဏစောင့်ပါ။\n\nပထမဆုံး Channel ကို Join လုပ်ပြီး Request ကိုကြည့်ပါ။</b>", reply_markup=InlineKeyboardMarkup(btn))
...
@Client.on_message(filters.command("shorrrrrtlink"))
...
@Client.on_message(filters.command("setshortlirrnkoff"))
...
@Client.on_message(filters.command("srretshortlinkon"))
...
@Client.on_message(filters.command("shortlink_irrrrnfo"))
...
@Client.on_message(filters.command("serrret_tutorial"))
...
@Client.on_message(filters.command("retttttmove_tutorial"))
    ...
    if len(message.command) == 2:
      ...
      #  time = message.command[2]
        ...
        if seconds > 0:
            ...
            await client.send_message(
                chat_id=user_id,
                text="<b>အက်မင်များမှ သင့် ပရီမီယံဖြုတ်သိမ်းလိုက်ပါပြီး\n\nဤအရာသည် အမှားတစ်ခုဖြစ်ပါက အက်မင်ကို ဆက်သွယ်ပါ\n\n👮 အက်မင်: {} \n</b>".format(OWNER_LNK),                
            )
        else:
...
async def plans_cmd_handler(client, message): 
    if PREMIUM_AND_REFERAL_MODE == False:
        return 
    btn = [            
        [InlineKeyboardButton("ငွေလွှဲပြေစာ ပေးပို့ရန် 🧾", url=OWNER_LNK)],
        [InlineKeyboardButton("⚠️ ပိတ် / ဖျက် ⚠️", callback_data="close_data")]
    ]
    reply_markup = InlineKeyboardMarkup(btn)
    await message.reply_photo(
        photo=PAYMENT_QR,
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        await message.reply_text(f"**သင့် Premium အသေးစိတ်များ:\n\nကျန်ရှိသော အချိန်: {remaining_time}\n\nသက်တမ်းကုန်ဆုံးမည့် အချိန်: {expiry_time}**")
    else:
        btn = [ 
            [InlineKeyboardButton("အခမဲ့ ၅ မိနစ် စမ်းသပ်ကြည့်ရှုခွင့် ရယူပါ ☺️", callback_data="get_trail")],
            [InlineKeyboardButton("Premium ဝယ်ယူရန် / ကြော်ငြာများ ဖယ်ရှားရန်", callback_data="buy_premium")],
            [InlineKeyboardButton("⚠️ ပိတ် / ဖျက် ⚠️", callback_data="close_data")]
        ]
        reply_markup = InlineKeyboardMarkup(btn)
        m=await message.reply_sticker("CAACAgEAAxkBAAEPz5lpHKOshHgqqkJxuYgCzdVpz8ZdugACOQIAAm6B2USQaKQpynaQzDYE")         
        await message.reply_text(f"**😢 သင့်မှာ ပရီမီယံ မရှိပါ။\n\n Premium ဝယ်ယူရန် /plan တွင် ကြည့်ရှုနိုင်ပါသည်။**",reply_markup=reply_markup)
